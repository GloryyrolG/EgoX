...
# 0.3.9 - expand ~ in paths
# 0.4.0 - allow users to set NOTARY_VERSION
# 0.4.1 - default script to be non-interactive; replaced -y/--no-prompt option with -i/--interactive
# 0.4.2 - upgrade notarycli version (token issue cache fix when used in bolt/lightning tasks)
# 0.4.3 - upgrade notarycli version (security fix for token issue cache permission)
# 0.4.4 - upgrade notarycli version to 1.19.0 (CVE-2024-24790)
# 0.4.5 - useability improvements
# 0.4.6 - upgrade notarycli version to 1.26.3. Switch to cask-based installation
# 0.4.7 - fix notarycly installation
# 0.4.8 - add endpoint_url to generated profiles
# 0.4.9 - fix internal endpoints
SCRIPT_VERSION=0.4.8

DEFAULT_TARGET_BIN_DIR="$HOME/.conductor/bin"
...
while [[ $# -gt 0 ]]; do
  case $1 in
    -h|--help)
      ...
      echo "-i, --interactive    Run script in interactive mode with prompts"
...
INSTALL_CONDUCTOR_INTERNAL_ENVIRONMENTS=${INSTALL_CONDUCTOR_INTERNAL_ENVIRONMENTS:-"false"}

if [[ "$ASSUME_YES" == "false" ]]; then
  echo "ℹ️ Running with --interactive"
fi
...
if ! is_installed aws; then
    echo "ℹ️ Going to install AWS CLI"
    if [ "$ASSUME_YES" = false ]; then
      read -rp "Would you like us to install awscli for you? (y/n): " confirm && [[ $confirm == [yY] || $confirm == [yY][eE][sS] ]] || exit 3
    fi

    manual_install=true
    if [[ $(command -v $BREW_COMMAND) != "" ]]; then
      if [[ $($BREW_COMMAND -v | head -1) == *-apple* ]]; then
        $BREW_COMMAND install awscli
    ...
    if [ "$manual_install" = true ]; then
      echo -e "❌ ${tty_red}AWS CLI.${tty_reset}\nPlease install by following the instructions here: ${tty_bold}https://docs.aws.amazon.com/cli/latest/userguide/getting-started-install.html${tty_reset}, then re-run this script."
      exit 1
    fi
else
  echo "✅ AWS CLI found"
...
if [[ $FORCE_INSTALL == "true" ]] || ! is_installed "$NOTARYCLI"; then
    ...
    if [[ $(command -v $BREW_COMMAND) != "" ]]; then
      if [[ $($BREW_COMMAND -v | head -1) == *-apple* ]]; then
        $BREW_COMMAND install --cask apple/turi/notarycli
    ...
    if [ "$install_with_curl" = true ]; then
      ...
      curl -Ls "https://artifacts.apple.com/polymer-generic-local/notary/notarycli/${NOTARY_VERSION}/${binary}.tar.gz" -o "/tmp/${binary}.tar.gz" && \
        ...
        chmod +x "/tmp/${NOTARYCLI}" && \
        mv "/tmp/${NOTARYCLI}" "${TARGET_BIN_DIR}"/${NOTARYCLI}
      NOTARYCLI_FULLPATH="${TARGET_BIN_DIR}"/${NOTARYCLI}
    fi
    echo "✔️ notarycli ${NOTARY_VERSION} installed"
else
...
region = conductor
endpoint_url = https://conductor-dev.data.apple.com

[profile conductor-notary-infra]
ca_bundle = ${APPLE_CA}
credential_process = ${NOTARYCLI_FULLPATH} issue -o conductor --audience=aprn:apple:turi::notary:application:conductor --conductor-endpoint https://conductor.infra.aws.usw2.g.apple.com
region = conductor
endpoint_url = https://conductor.infra.aws.usw2.g.apple.com

[profile conductor-notary-ut]
ca_bundle = ${APPLE_CA}
credential_process = ${NOTARYCLI_FULLPATH} issue -o conductor --audience=aprn:apple:turi::notary:application:conductor --conductor-endpoint https://conductor.ut.aws.usw2.g.apple.com
region = conductor
endpoint_url = https://conductor.ut.aws.usw2.g.apple.com
...
region = conductor
endpoint_url = https://conductor-stg.data.apple.com
...
# added automatically via the setup-conductor.sh script
set -eu
AWS_EC2_METADATA_DISABLED=true AWS_CA_BUNDLE=${APPLE_CA} exec aws --profile conductor-notary-infra --endpoint-url https://conductor.infra.aws.usw2.g.apple.com "\$@"
END
chmod +x $TARGET_BIN_DIR/conductor-infra
echo "✔️ Installed conductor-infra"


cat > $TARGET_BIN_DIR/conductor-ut <<- END
#!/bin/bash
# added automatically via the setup-conductor.sh script
set -eu
AWS_EC2_METADATA_DISABLED=true AWS_CA_BUNDLE=${APPLE_CA} exec aws --profile conductor-notary-ut --endpoint-url https://conductor.ut.aws.usw2.g.apple.com "\$@"
END
chmod +x $TARGET_BIN_DIR/conductor-ut
echo "✔️ Installed conductor-ut"
